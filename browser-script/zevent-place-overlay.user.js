...
// @match http://localhost:63342/zevent-zplace-overlays/test/index.html?_ijt=jhu9guerl9kgaj6nsehq227ps2&_ij_reload=RELOAD_ON_SAVE
 ...
 * Copyright 2021-2025 ludolpif, ventston, PiRDub
...
const symbolsUrl = "https://overlay-zplace.4each.dev/symbols.json";
let SYMBOL_H = 5;
let SYMBOL_W = 5;
let colors = [];
...
(function () {
    ...
    let lastCustomId = 0;
    let symbols = [];
    let enableSymbols = GM_getValue("enableSymbols", false);
    ...
    function fitOverlayOnCanvas(origCanvas, image) {
        ...
        if ((nw % 300) || (nh % 300)) {
            ...
            if ((nw % 7 === 0) && (nh % 7 === 0)) {
                ...
                zpoLog("fitOverlayOnCanvas() WARNING: adding image size that is not multiple of 300 or 7, badly exported overlay");
    ...
    function appendOurUI(origUI) {
        ...
        if (enableSymbolsCheckbox) {
            enableSymbolsCheckbox.checked = enableSymbols;
            enableSymbolsCheckbox.onchange = async function (event) {
                enableSymbols = event.target.checked;
                GM_setValue('enableSymbols', enableSymbols);
                if (enableSymbols) {
    ...
    //inject symbols in color palette
    ...
    const getColors = async () => {
        ...
        if(!loadedColors || loadedColors.length === 0) {
            ...
            colors = [
                {colorCode: '#000000'}, {colorCode: '#333434'},
                {colorCode: '#D4D7D9'}, {colorCode: '#FFFFFF'},
                {colorCode: '#6D302F'}, {colorCode: '#9C451A'},
                {colorCode: '#6D001A'}, {colorCode: '#BE0027'},
                {colorCode: '#FF2651'}, {colorCode: '#FF2D00'},
                {colorCode: '#FFA800'}, {colorCode: '#FFB446'},
                {colorCode: '#FFD623'}, {colorCode: '#FFF8B8'},
                {colorCode: '#7EED38'}, {colorCode: '#00CC4E'},
                {colorCode: '#00A344'}, {colorCode: '#598D5A'},
                {colorCode: '#004B6F'}, {colorCode: '#009EAA'},
                {colorCode: '#00CCC0'}, {colorCode: '#33E9F4'},
                {colorCode: '#5EB3FF'}, {colorCode: '#245AEA'},
                {colorCode: '#313AC1'}, {colorCode: '#1832A4'},
                {colorCode: '#511E9F'}, {colorCode: '#6A5CFF'},
                {colorCode: '#33E9F4'}, {colorCode: '#B44AC0'},
                {colorCode: '#FF63AA'}, {colorCode: '#E4ABFF'}
            ];
        } else {
            colors = loadedColors;
        }
    ...
    const initSymbols = async () => {
        if (enableSymbols && !paletteObserver) {
